...
-- Lua 5.4 removed math.pow; restore it for Roblox script compatibility.
if not math.pow then
    math.pow = function(Base, Exp) return Base ^ Exp end
end

-- Override os.clock for the Nyx re-run-per-tick model.
-- Every tick re-runs the full script from scratch, so `local t = os.clock()` at
-- module scope would recapture the CPU clock every tick, making
-- `elapsed = os.clock() - t` always ≈ 0 and freezing all time-based animation.
-- Fix: first call returns 0 (acts as the scene-start origin), subsequent calls
-- return _NYX_LIVE_TIME so that `elapsed = os.clock() - t` equals scene time.
...
function Part:Destroy()
    -- Viewport destroy is not live yet; safe no-op.
end
...
-- ── Workspace ─────────────────────────────────────────────────────────────────
-- Stored as a pure-proxy table so __newindex fires for every assignment,
-- including re-assignments to Gravity. This also keeps the object identity
-- stable so Part proxies can compare V == workspace.

local _WorkspaceData     = { Gravity = 196.2 }
local _WorkspaceChildren = {}
local _WorkspaceMethods  = {}
local _NyxCameraObj      -- forward-declared; defined below
...
function _WorkspaceMethods.AddPart(self, P)
    ...
    -- BrickColor takes priority over Color when both are present
...
-- ── Part proxy (property-intercept wrapper) ───────────────────────────────────
...
do
    ...
    function Part.new()
        ...
        return setmetatable({}, {
            __index    = D,
            __newindex = function(_, K, V)
                -- BrickColor syncs through to Color immediately
...
function Instance.new(ClassName)
    ...
    -- Unknown class: return a silent stub that won't throw on property access
